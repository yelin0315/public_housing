...
independent_vars <- c("d.hos", "size", "rate", "value", "d.sub", "security", "avg.price", "supply")

# Create individual scatterplots
for (var in independent_vars) {
  gg <- ggplot(data, aes_string(x = var, y = "cut")) +
    geom_point(aes(color = cut)) +
    labs(x = var, y = "cut", title = paste("Scatterplot of", var, "vs. cut"))
